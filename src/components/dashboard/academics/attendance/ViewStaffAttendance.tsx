...
const ViewStaffAttendance = ({ user }: { user: any }) => {
  ...
  const activeSchool = authUser?.schools[0];

  const canViewAll = user?.roles.includes(UserRole.ADMIN) || user?.roles.includes(UserRole.SUPER_ADMIN);
  ...
  const handleFetchAttendance = useCallback(() => {
    ...
    if (canViewAll) {
      ...
      params.staffId = authUser?.staff?.id;
      params.month = month;
      params.year = year;
    }

    if(params.staffId || (params.month && params.year) || params.date) {
  ...
  }, [canViewAll, selectedStaff, date, authUser, month, year]);

  useEffect(() => {
    if (canViewAll && activeSchool) {
        ...
        {canViewAll ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <Select value={year.toString()} onValueChange={(val) => setYear(parseInt(val))}>
                <SelectTrigger><SelectValue/></SelectTrigger>
                <SelectContent>
                    {Array.from({length: 10}, (_, i) => new Date().getFullYear() - i).map(y => <SelectItem key={y} value={y.toString()}>{y}</SelectItem>)}
                </SelectContent>
            </Select>
          </div>
        )}
