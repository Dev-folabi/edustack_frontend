...
const invoiceSchema = z.object({
  ...
  assignmentType: z.enum(['SINGLE_STUDENT', 'MULTIPLE_STUDENTS', 'CLASS', 'SECTION']),
...
const CreateInvoicePage = () => {
  ...
  const { sessions, fetchSessions, terms, fetchTerms, selectedSession } = useSessionStore();
  ...
  useEffect(() => {
    if (selectedSchool) {
      ...
      financeService.getFeeCategories(selectedSchool.schoolId, 1, 100).then(res => setFeeCategories(res.data.data));
      studentService.getStudentsBySchool(selectedSchool.schoolId, { limit: 1000 }).then(res => setStudents(res.data.data));
    }
  }, [selectedSchool]);
  ...
  const onSubmit = async (data: InvoiceFormData) => {
    ...
    try {
      ...
      showToast({ type: 'success', title: 'Success', message: 'Invoice created successfully' });
      router.push('/finance/fee-management');
    } catch (error: any) {
      showToast({ type: 'error', title: 'Error', message: error.message || 'Failed to create invoice' });
    }
  };
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                />
                {errors.termId && <p className="text-red-500 text-sm">{errors.termId.message}</p>}
              </div>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent>
                          {feeCategories.map(cat => <SelectItem key={cat.id} value={cat.id}>{cat.name}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    )}
                  />
                  <Input type="number" {...register(`items.${index}.amount`, { valueAsNumber: true })} placeholder="Amount" />
                  <Button type="button" variant="destructive" onClick={() => remove(index)}><Trash2 className="h-4 w-4" /></Button>
                </div>
              ))}
              <Button type="button" onClick={() => append({ feeCategoryId: '', amount: 0 })}><Plus className="mr-2 h-4 w-4" />Add Item</Button>
            ...
            <div>
                ...
                render={({ field }) => (
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="MULTIPLE_STUDENTS">Multiple Students</SelectItem>
              ...
              />
              {errors.assignmentType && <p className="text-red-500 text-sm">{errors.assignmentType.message}</p>}
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-4">
                      ...
                      <div>
                          ...
                          <Controller
                            ...
                            name="sectionId"
                            render={({ field }) => (
                              <Select onValueChange={field.onChange} defaultValue={field.value} disabled={!watch('classId')}>
                                <SelectTrigger><SelectValue placeholder="Select a section" /></SelectTrigger>
                                <SelectContent>
                                  {classes.find(c => c.id === watch('classId'))?.sections.map(s => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}
                                </SelectContent>
                              </Select>
                            )}
                          />
...
export default withAuth(CreateInvoicePage, [UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.FINANCE]);
