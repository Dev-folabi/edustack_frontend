...
const createFormSchema = z.object({
  ...
  sections: z.array(z.object({ name: z.string().min(1, "Section name is required") })),
});
...
const EditClassModal = ({ isOpen, onClose, classData, onClassUpdated }: { isOpen: boolean, onClose: () => void, classData: Class | null, onClassUpdated: () => void }) => {
    const { updateClass } = useClassStore();
    const { register, handleSubmit, control, formState: { errors }, reset } = useForm<z.infer<typeof editFormSchema>>({
...
const DeleteClassDialog = ({ isOpen, onClose, onConfirm, className }: { isOpen: boolean, onClose: () => void, onConfirm: () => void, className: string }) => {
    return (
        <AlertDialog open={isOpen} onOpenChange={onClose}>
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        This action cannot be undone. This will permanently delete the class "{className}" and all of its sections.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
...
const CreateClassModal = ({ isOpen, onClose, onClassCreated }: { isOpen: boolean, onClose: () => void, onClassCreated: () => void }) => {
    const { schools, fetchSchools } = useSchoolStore();
    const { createClass } = useClassStore();
    const { register, handleSubmit, control, formState: { errors } } = useForm<z.infer<typeof createFormSchema>>({
...
const ClassesPage = () => {
  ...
  const { classes, isLoading, error, fetchClasses, deleteClass } = useClassStore();
  ...
  return (
    <div className="container mx-auto p-4">
      <Card>
        <CardHeader>
          <CardTitle className="flex justify-between items-center">
            <span>Classes</span>
            <Button onClick={() => setCreateModalOpen(true)}>Add New Class</Button>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                {classes.map((classItem) => (
                  <React.Fragment key={classItem.id}>
                    <TableRow>
                      ...
                      <TableCell>
                        <Button variant="outline" size="sm" onClick={() => handleEditClass(classItem)}>Edit</Button>
                        <Button variant="destructive" size="sm" className="ml-2" onClick={() => handleDeleteClass(classItem)}>Delete</Button>
                      </TableCell>
                    </TableRow>
                  </React.Fragment>
                ))}
      ...
      <CreateClassModal isOpen={isCreateModalOpen} onClose={() => setCreateModalOpen(false)} onClassCreated={handleClassCreated} />
      <EditClassModal isOpen={isEditModalOpen} onClose={() => setEditModalOpen(false)} classData={selectedClass} onClassUpdated={handleClassUpdated}/>
      <DeleteClassDialog isOpen={isDeleteDialog} onClose={() => setDeleteDialog(false)} onConfirm={confirmDelete} className={selectedClass?.name || ''} />
    </div>
  );
};
