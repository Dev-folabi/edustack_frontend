...
const SessionsDashboardPage = () => {
  ...
  const handleToggleActive = async (sessionToToggle: Session) => {
    ...
    const currentlyActive = sessions.find(
      (s) => s.isActive && s.id !== sessionToToggle.id
    );
    ...
    try {
      ...
      await sessionService.updateSession(
        sessionToToggle.id,
        { isActive: !sessionToToggle.isActive },
        selectedSchool.schoolId
      );
  ...
  const handleDeleteSession = async () => {
    if (!sessionToDelete || !selectedSchool?.schoolId) return;
    try {
  ...
  const handleToggleExpand = async (sessionId: string) => {
    ...
    if (!isExpanded && !sessionTerms[sessionId]) {
      ...
      try {
        ...
        showToast({
          title: "Error",
          message: "Failed to fetch terms for the session.",
          type: "error",
        });
      } finally {
        ...
        <CardHeader className="flex flex-row justify-between items-start">
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
              ...
              />
              <label
                htmlFor={`active-switch-${session.id}`}
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This action will delete the session and all its associated terms.
...
export default withAuth(SessionsDashboardPage, [
